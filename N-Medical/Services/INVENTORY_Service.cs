...
namespace N_Medical.Services
{
    public class INVENTORY_Service : Service_SQL
    {
        ...
        public DataTable getDataBySaleOrder(string saleOrderNumber)
        {
            ...
            SQL.CommandText = "Select Id, Consumtion_Number, Invoice_Number, Credit_Note, PO_Number, SALE_ORDER_Number, Cus_Group_Code, Cus_Group_Name, Cus_Code, Cus_Name, Dept_Code, Dept_Name, Sale_Code, Sale_Name, Cus_Shipping_Code, Cus_Shipping_Detail, Collector, Delivery_Route_Code, Delivery_Route_Name, Warehouse_Code, Warehouse_Name, Invoice_Remark, Invoice_Discount, Invoice_Date, CONVERT(VARCHAR(10),Invoice_Date,103) as INV_Date, Invoice_Status, case when Invoice_Status = 'C' then 'Complete' when Invoice_Status = 'N' then 'Credit Note' else 'On Process' end as Status_Name, INVOICE.User_Create, INVOICE.User_Update, INVOICE.Create_Date, INVOICE.Update_Date from INVOICE where SALE_ORDER_Number = @saleOrderNumber Order By Id desc";
            return this.ExecuteReader(this.SQL);
        }
        ...
        public DataTable checkAddConsumtion(string consumtion, string status)
        {
            SQL.Parameters.Add("@consumtion", SqlDbType.NVarChar).Value = consumtion;
            SQL.CommandText = "Select * from INVOICE Where Consumtion_Number = @consumtion and Invoice_Status in ('" + @status + "')";
            return this.ExecuteReader(this.SQL);
        }
        public DataTable checkAddConsumtionDetail(string consumtion, string status)
        {
            SQL.Parameters.Add("@consumtion", SqlDbType.NVarChar).Value = consumtion;
            SQL.CommandText = "Select * from INVOICE_Detail Where Consumtion_Number = @consumtion and Status in ('" + @status + "')";
            return this.ExecuteReader(this.SQL);
        }
        public DataTable checkEditConsumtion(string id, string consumtion, string status)
        {
            SQL.Parameters.Add("@id", SqlDbType.BigInt).Value = id;
            SQL.Parameters.Add("@consumtion", SqlDbType.NVarChar).Value = consumtion;
            SQL.CommandText = "Select * from INVOICE Where Id <> @id and Consumtion_Number = @consumtion and Invoice_Status in ('" + @status + "')";
            return this.ExecuteReader(this.SQL);
        }
        public DataTable checkEditConsumtionDetail(string id, string consumtion, string status)
        {
            SQL.Parameters.Add("@id", SqlDbType.BigInt).Value = id;
            SQL.Parameters.Add("@consumtion", SqlDbType.NVarChar).Value = consumtion;
            SQL.CommandText = "Select * from INVOICE_Detail Where Invocie_Id <> @id and Consumtion_Number = @consumtion and Status in ('" + @status + "')";
            return this.ExecuteReader(this.SQL);
        }
        ...
        public DataTable insertInvoiceData(string cusGroupCode, string cusGroupName, string cusCode, string cusName, string deptCode, string deptName, string consumtionNo, string poNo, string cusShippingCode, string cusShippingDetail, string saleCode, string saleName, string deliveryRouteCode, string deliveryRouteName, string warehouseCode, string warehouseName, string invoiceRemark, string invoiceDiscount, string invoiceStatus, string invoiceType, string saleOrderNumber, string userCreate, string userUpdate)
        {
            ...
            SQL.CommandText = "Insert Into Invoice (Cus_Group_Code, Cus_Group_Name, Cus_Code, Cus_Name, Dept_Code, Dept_Name, Consumtion_Number, PO_Number, Cus_Shipping_Code, Cus_Shipping_Detail, Sale_Code, Sale_Name, Delivery_Route_Code, Delivery_Route_Name, Warehouse_Code, Warehouse_Name, Invoice_Remark, Invoice_Discount, Invoice_Status, Invoice_Type, SALE_ORDER_Number, User_Create, User_Update, Create_Date, Update_Date) Values (@cusGroupCode, @cusGroupName, @cusCode, @cusName, @deptCode, @deptName, @consumtionNo, @poNo, @cusShippingCode, @cusShippingDetail, @saleCode, @saleName, @deliveryRouteCode, @deliveryRouteName, @warehouseCode, @warehouseName, @invoiceRemark, @invoiceDiscount, @invoiceStatus, @invoiceType, @saleOrderNumber, @userCreate, @userUpdate, GETDATE(), GETDATE());SELECT SCOPE_IDENTITY() AS insertId;";
            return this.ExecuteReader(this.SQL);
        }
        public void insertInvoiceDetailData(string invoiceId, string consumtionNo, string itemCode, string itemName, string itemLocation, string batch, string expireDate, string itemCost, string itemPrice, string itemQty, string uom, string costCenter, string status, string userCreate)
        {
            SQL.Parameters.Add("@invoiceId", SqlDbType.BigInt).Value = invoiceId;
            SQL.Parameters.Add("@consumtionNo", SqlDbType.NVarChar).Value = consumtionNo;
            ...
            SQL.Parameters.Add("@userCreate", SqlDbType.NVarChar).Value = userCreate;
            SQL.CommandText = "Insert Into Invoice_Detail (Invoice_Id, Consumtion_Number, Item_Code, Item_Name, Item_Location, Batch, Expire_Date, Item_Cost, Item_Price, Item_Qty, UOM, Cost_Center, Status, User_Create, Create_Date, User_Update, Update_Date) Values (@invoiceId, @consumtionNo, @itemCode, @itemName, @itemLocation, @batch, @expireDate, @itemCost, @itemPrice, @itemQty, @uom, @costCenter, @status, @userCreate, GETDATE(), @userCreate, GETDATE())";
